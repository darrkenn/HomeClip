...
def getLinksInTag(selectedTag):
    ...
    c = conn.cursor()
    c.execute("SELECT l.LinkId, l.Title, l.Link, t.Tag, f.Folder FROM Links as l LEFT JOIN main.Tags t on l.TagId = t.TagId LEFT JOIN main.Folders f on f.FolderId = l.FolderId WHERE t.Tag LIKE ?", (selectedTag,))
    values = c.fetchall()
    conn.close()
